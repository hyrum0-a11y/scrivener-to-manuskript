...
**`obsidian_to_odt.py`** — compiles the same vault into an ODT (OpenDocument Text) manuscript via pandoc, styled to match the PDF as closely as ODT reasonably allows: a real title page, centered/hidden front matter, centered POV names, chapter dates, epigraphs, drop caps, a page-number-and-title header (no footer), and every front/back-matter item, Part, and chapter starting on its own page — via pandoc's `custom-style` mechanism (a div/span attribute that maps onto a real named ODF paragraph/character style — unlike CSS-style `.classname` divs, which pandoc's ODT writer silently drops). Has its own independent renderer (not reused from `obsidian_to_epub.py`, unlike an earlier version) since ODT styling works fundamentally differently from epub's CSS classes. One known, unresolved gap versus the PDF: page numbers run continuously from the title page rather than restarting at 1 on the manuscript body — a mid-document page-style switch is the spec-correct ODF way to do that, and an attempt is included, but isolated testing across four configurations never got LibreOffice's headless PDF export to honor it (untested in interactive Writer). Meant as an editable draft format for further work in a word processor, not a finished distributable.
```
