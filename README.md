...
- For `obsidian_to_odt.py` only: a native LibreOffice install (provides both `soffice` and the `uno` Python module — a Flatpak install does not expose `uno` to the system Python and is not supported) and `pymupdf`
